...
public class GetConnectedComponents {
	...
	public static ArrayList<ArrayList<Pair<Integer, Integer>>> getComponentsWithKnownCells(VisibleTile[][] board) throws Exception {
		...
		for (int i = 0; i < rows; ++i) {
			for (int j = 0; j < cols; ++j) {
				...
				dfs(i, j, component, visited, hasAdjacentNumber);
				components.add(component);
			}
		}
		return components;
	...
	private static void dfs(int i, int j, ArrayList<Pair<Integer, Integer>> component, boolean[][] visited, boolean[][] hasAdjacentNumber) {
		component.add(new Pair<>(i, j));
		visited[i][j] = true;
		for (int[] adj : GetAdjacentCells.getAdjacentCells(i, j, rows, cols)) {
			final int adjI = adj[0], adjJ = adj[1];
			if (visited[adjI][adjJ] || !hasAdjacentNumber[adjI][adjJ]) {
				continue;
			}
			dfs(adjI, adjJ, component, visited, hasAdjacentNumber);
		}
	}
