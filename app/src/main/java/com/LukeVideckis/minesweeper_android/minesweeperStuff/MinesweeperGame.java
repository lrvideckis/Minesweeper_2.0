...
public class MinesweeperGame {
	...
	public void changeMineStatusAroundRandomClue(boolean flipType) throws Exception {
		boolean[][] interestingClue = new boolean[rows][cols];
		for (int i = 0; i < rows; ++i) {
			for (int j = 0; j < cols; ++j) {
				if (isInterestingCell(i, j) &&
					...
					for (int[] adj : GetAdjacentCells.getAdjacentCells(i, j, rows, cols)) {
						...
						if (getCell(adjI, adjJ).isVisible) {
							interestingClue[adjI][adjJ] = true;
						}
					}
				}
			}
		...
		ArrayList<Pair<Integer, Integer>>
				spotsWithFree = new ArrayList<>(),
				spotsWithMine = new ArrayList<>(),
				awayMines = new ArrayList<>(),
				awayFrees = new ArrayList<>();
		for (int i = 0; i < rows; ++i) {
			for (int j = 0; j < cols; ++j) {
				if (interestingClue[i][j]) {
					boolean hasAdjMine = false, hasAdjFree = false;
					for (int[] adj : GetAdjacentCells.getAdjacentCells(i, j, rows, cols)) {
						final int adjI = adj[0], adjJ = adj[1];
						if (!getCell(adjI, adjJ).isVisible) {
							if (getCell(adjI, adjJ).isMine) {
					...
					if (hasAdjMine) {
						spotsWithMine.add(new Pair<>(i, j));
					}
					if (hasAdjFree) {
						spotsWithFree.add(new Pair<>(i, j));
					}
				}
				if (AwayCell.isAwayCell(this, i, j)) {
					if (getCell(i, j).isMine) {
						awayMines.add(new Pair<>(i, j));
					} else {
						awayFrees.add(new Pair<>(i, j));
					}
				}
			}
		}
		if (spotsWithMine.isEmpty() || spotsWithFree.isEmpty()) {
			throw new Exception("no spot to change mine status of");
		}

		boolean fillWithMines = (numberOfMines / (double) (rows * cols) < awayMines.size() / (double) (awayMines.size() + awayFrees.size()));
		if (flipType) {
			fillWithMines = !fillWithMines;
		}

		Collections.shuffle(spotsWithMine);
		Collections.shuffle(spotsWithFree);
		Collections.shuffle(awayMines);
		int i, j;
		if (fillWithMines) {
			i = spotsWithFree.get(0).first;
			j = spotsWithFree.get(0).second;
		} else {
			i = spotsWithMine.get(0).first;
			j = spotsWithMine.get(0).second;
		}
		int cntMines = 0, cntFrees = 0;
		for (int[] adj : GetAdjacentCells.getAdjacentCells(i, j, rows, cols)) {
			final int adjI = adj[0], adjJ = adj[1];
			if (getCell(adjI, adjJ).isVisible) {
				continue;
		...
		if (fillWithMines && cntFrees > awayMines.size()) {
			throw new Exception("not enough away mines");
		}
		if (!fillWithMines && cntMines > awayFrees.size()) {
			throw new Exception("not enough away frees");
		}
		int posAway = 0;
		System.out.println("pos of clue to change: " + i + " " + j);
		for (int[] adj : GetAdjacentCells.getAdjacentCells(i, j, rows, cols)) {
			final int adjI = adj[0], adjJ = adj[1];
			if (getCell(adjI, adjJ).isVisible) {
				continue;
			...
			if (fillWithMines) {
				if (!getCell(adjI, adjJ).isMine) {
					System.out.println("moving a mine from: " + awayMines.get(posAway).first + "," + awayMines.get(posAway).second + " to: " + adjI + "," + adjJ);
					changeMineStatus(adjI, adjJ, true);
					changeMineStatus(awayMines.get(posAway).first, awayMines.get(posAway).second, false);
					++posAway;
				}
			} else {
				if (getCell(adjI, adjJ).isMine) {
					System.out.println("moving a mine from: " + adjI + "," + adjJ + " to: " + awayMines.get(posAway).first + "," + awayMines.get(posAway).second);
					changeMineStatus(adjI, adjJ, false);
					changeMineStatus(awayFrees.get(posAway).first, awayFrees.get(posAway).second, true);
					++posAway;
				}
			}
		}
